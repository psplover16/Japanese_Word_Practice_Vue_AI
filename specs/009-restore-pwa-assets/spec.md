...
- **FR-003**: 系統必須確保桌面 icon 與 PWA 圖示在建置後包含於可發布產物中，而不是只存在於私人參考目錄；本次正式公開來源以根目錄 `public/` 為準，`_private/_private_fileAssets/v1/public` 僅作為原始參考資產位置。
...
- **公開資產來源**: 建置與部署時真正會被帶入公開站點的資產來源位置；本次以根目錄 `public/` 為正式來源，用來區分私人參考檔與可發布檔。
...
- 使用者提到的 icon 與 PWA 圖示檔案是本次應優先採用的現有參考素材，正式發布版本會複製到根目錄 `public/`。
