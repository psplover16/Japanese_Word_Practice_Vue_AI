...
- **FR-012A**: 系統 MUST 讓敬體基礎相關內容的前兩個核心區塊固定維持「`敬體變化速覽` 在前、`敬體句型：現在型與詞類基礎` 在後」的順序；兩個區塊都必須維持與其他 N5 文法群組一致的預設收合行為。
- **FR-012B**: `敬體變化速覽` MUST 使用 compare table 呈現名詞 / な形容詞、い形容詞與動詞的敬體對照，且每個表格儲存格都必須至少附一組含日文、讀音與繁體中文翻譯的例句；若同格含替代寫法，可在同一群組內以註記補充。
- **FR-012C**: `敬體句型：現在型與詞類基礎` MUST 保留原 `sentence-basics` 的教學主題、既有例句與共通提醒，但不得再重複顯示第二份敬體 compare table。
- **FR-012D**: 與第二區塊銜接的下一個核心區塊 MUST 以 `敬體句型：過去、狀態與補充表現` 命名，清楚表達其屬於敬體句型的延伸整理。
...
- **SC-006**: `/n5-grammar` 第一個區塊固定為 `敬體變化速覽`，展開後可見 compare table 與 12 組依「名詞 / な形容詞 → い形容詞 → 動詞」排序的對應例句；第二個區塊固定為 `敬體句型：現在型與詞類基礎`，且不重複顯示 compare table。
