...
作為使用 375px 手機寬度的使用者，我希望 `/practice` 中本期調整的表格與共享 route tabs 在窄螢幕上仍然可讀且不破版，讓我不需要左右捲動也不會看到內容擠壓重疊。

**Why this priority**: 這是明確指定的裝置場景，也是本期版面調整是否真正可用的最後一道驗收。

**Independent Test**: 將 viewport 設為 375px 後打開 `/practice`，確認共享 route tabs、長音規則、拗音與外來語矩陣都能維持完整可讀。

**Acceptance Scenarios**:

1. **Given** 使用者以 375px 寬度開啟 `/practice`，**When** 檢視共享 route tabs 與分頁區，**Then** 不得出現內容裁切、重疊或橫向捲動。
...
- **FR-016**: 系統 MUST 在 375px 寬度下維持 `/practice` 共享 route tabs、分頁區與本期調整表格的完整可讀性，不得出現裁切、重疊或水平捲動。
...
- **Practice 版面狀態**: 代表 `/practice` 在初始渲染與 375px 小螢幕條件下，共享 route tabs、表格與下半部區塊應維持的穩定呈現狀態。
...
- **SC-003**: 在 375px 寬度下檢視 `/practice` 時，本期調整的共享 route tabs 與表格區塊不得出現水平捲動、內容裁切或文字重疊。
