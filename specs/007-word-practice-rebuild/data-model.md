...
- Purpose: 代表由原始字典資料導出的 stage 中繼分組，主要用於資料驗證、順序檢查與測試，不是目前畫面的主要渲染容器。
- Fields:
  - `stage`: stage 名稱。
...
- Purpose: 代表套用目前 filter 後的 stage 可見資料導出結果，供測試、分析或後續擴充使用；現行畫面仍以單一 table 呈現。
- Fields:
  - `stage`: stage 名稱。
  - `entries[]`: 該 stage 的完整 `VocabularyEntry`。
...
- Purpose: 代表單一資料列目前是否被註記與對應提示狀態；實作上由 `persistedMarkedIds` / `draftMarkedIds` 等狀態共同表達。
- Fields:
...
- Fields:
  ...
  - `active`: 是否處於揭露中。
  - `startedAt`: 開始計時時間。
  - `thresholdMs`: 觸發揭露的長按門檻，預計約 400ms。

## Derived View Rules

- `VocabularyEntry` 的 `id` 必須依原始字典順序穩定產生，後續註記持久化只儲存此值。
- `VisibleVocabularyStageGroup.visibleEntries[]` 由 `VocabularyFilterState`、`VocabularyMarkSnapshot` 與 `/practice` 勾選狀態共同決定，但目前畫面主渲染仍以單一 table 呈現。
...
- stage metadata 順序必須依原始資料首次出現順序固定，不可在 runtime 任意排序。
